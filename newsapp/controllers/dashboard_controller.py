...
def my_articles():
    ...
    articles = query.all()
    return render_template("dashboard/articles.html", user=user, articles=articles, q=q, status=status, sort=sort)
...
def create_article():
    ...
    return render_template("dashboard/article_form.html", categories=categories, article=None, mode="create")
...
def edit_article(article_id: int):
    ...
    categories = Category.query.filter_by(active=True).order_by(Category.name.asc()).all()
    return render_template("dashboard/article_form.html", article=article, categories=categories, mode="edit")
